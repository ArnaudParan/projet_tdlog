...
def create_event(request):
    email = request.GET['email']
    pwd = request.GET['mdp_hache']
    latitude = request.GET['latitude']
    longitude = request.GET['longitude']
    nom = request.GET['nom']
    #date = request.GET['date']
    id_createur = request.GET['id']
    createur0=Utilisateur.objects.filter(identifiant=id_createur).first()
    ...
    if is_user(email,pwd):
        ...
        for j in range(1,nb_participants):
            ...
            participant = Utilisateur.objects.filter(identifiant=id_ami).first()
